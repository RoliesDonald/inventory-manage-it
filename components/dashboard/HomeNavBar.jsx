        ...
        <div className='bg-neutral-200 shadow-md h-auto'>
            <div className='flex space-x-3 p-3 items-center '>
                ...
                <div className="flex flex-col">
                    <p className='text-2xl roboto font-semibold text-slate-400'>Hallo, Manager Rent</p>
                    ...
                    {navLink.map((item, i) => {
                        return (
                            <Link
                                href={item.href}
                                key={i}
                                className={`${pathName === item.href ? "py-2 px-4 border-b-2 border-blue-600 text-slate-600" : "py-2 px-3"}`}
                            >
                                {item.title}
                            </Link>
                        )
